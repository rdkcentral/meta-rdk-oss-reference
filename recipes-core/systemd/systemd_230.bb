...
PACKAGECONFIG ??= "xz \
                   ${@oe.utils.conditional('USE_LDCONFIG', '1', 'ldconfig', '', d)} \
...
CACHED_CONFIGUREVARS:class-target = "\
...
do_configure:prepend() {
        sed -i -e 's/lnr/ln -s/g' ${S}/Makefile.am
}
...
USERADD_PARAM:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'microhttpd', '--system -d / -M --shell /bin/nologin systemd-journal-gateway;', '', d)}"
USERADD_PARAM:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'microhttpd', '--system -d / -M --shell /bin/nologin systemd-journal-remote;', '', d)}"
USERADD_PARAM:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'journal-upload', '--system -d / -M --shell /bin/nologin systemd-journal-upload;', '', d)}"
USERADD_PARAM:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'timesyncd', '--system -d / -M --shell /bin/nologin systemd-timesync;', '', d)}"
USERADD_PARAM:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'networkd', '--system -d / -M --shell /bin/nologin systemd-network;', '', d)}"
USERADD_PARAM:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'coredump', '--system -d / -M --shell /bin/nologin systemd-coredump;', '', d)}"
USERADD_PARAM:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'resolved', '--system -d / -M --shell /bin/nologin systemd-resolve;', '', d)}"
GROUPADD_PARAM:${PN} = "-r lock; -r systemd-journal; -r wheel"
USERADD_PARAM:${PN}-extra-utils += "--system -d / -M --shell /bin/nologin systemd-bus-proxy;"
...
RDEPENDS:${PN}-ptest += "gawk make perl bash xz \
...
FILES:${PN}-ptest += "${libdir}/udev/rules.d"
...
FILES:${PN}-xorg-xinitrc = "${sysconfdir}/X11/xinit/xinitrc.d/*"
...
CONFFILES:${PN} = "${sysconfdir}/machine-id \
...
RDEPENDS:${PN} += "kmod dbus util-linux-mount udev (= ${EXTENDPKGV})"
RDEPENDS:${PN} += "volatile-binds update-rc.d"

RRECOMMENDS:${PN} += "${@bb.utils.contains('PACKAGECONFIG', 'serial-getty-generator', '', 'systemd-serialgetty', d)} \
...
ALTERNATIVE:${PN} = "init halt reboot shutdown poweroff runlevel"
